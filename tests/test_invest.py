...
class TestInvest(unittest.TestCase):
    ...
    def test_invest(self, data_info):
        ...
        # 投资金额输入大于可投标金额
            ...
            query_6 = "select id from loan where status = 2 and (amount between 200 and 20000) limit 1"
            maxid = self.db.query(query_6)
            max_id = maxid["id"]
            data_info["data"] = data_info["data"].replace("#max_id", str(max_id))
        # 投资金额输入等于可投标金额,投标完成
        elif "#amount" in data_info["data"]:
            query_7 = "select amount from loan where id = {} limit 1".format(json.loads(data_info['data'])['loan_id'])
            amount = self.db.query(query_7)
        ...
        if data_info['yes_or_no'] == "YES":
            ...
            if original_amt < Decimal('80000'):
                ...
                logger.info("账户id为{}的账户原始金额是{}".format(self.member_id, original_amt))
                print(json.loads(data_info['data'])['amount'])
                sum_amt = original_amt - Decimal(json.loads(data_info['data'])['amount']) - sum_amt
        # 投资金额输入大于用户余额,余额不足
        ...
        try:
            ...
            if eval(data_info['expected']).get('msg'):
